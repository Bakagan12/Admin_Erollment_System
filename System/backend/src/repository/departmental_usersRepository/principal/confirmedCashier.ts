...
export class ConfirmedRegistrar{
 static async student_cashier_confirmed(): Promise<any> {
    try {
      const students = await db('students')
        ...
        .where('students.is_confirmed_cashier', 1)
      ...
      return {
        message: 'Confirmed students on cashier retrieved successfully',
        data: students
      };
    } catch (error) {
      console.error('Error retrieving Confirmed students on cashier', error);
      throw new Error('Failed to retrieve Confirmed students on cashier');
    }
  }
